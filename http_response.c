...
/* Initialize a very basic hw_http_response struct */
...
void
hw_set_http_version(hw_http_response *response, unsigned short major,
...
/* Since hw_http_response is opaque, we create setters here like this function
 * that first convert the opaque pointer to a non-opaque, and then do simple
 * "zero copy" of struct into struct. */
void
hw_set_response_status_code(hw_http_response *response, hw_string *status_code)
{
	http_response *resp = (http_response *)response;
	/* This is what kellabyte refers to as "Zero-copy" but it's just a struct
	 * copy done by the compiler. */
	resp->status_code = *status_code;
}

void hw_set_response_header(hw_http_response* response, hw_string* name, hw_string* value)
{
	/* Turn response into a non-opaque variable */ 
	http_response *resp = (http_response*)response;
	/* Grab a pointer to an empty header */
	struct http_header *header = &resp->headers[resp->number_of_headers];

	/* Actually set the values. Again these are struct copies.  */
	header->name = *name;
	header->value = *value;

	/* Copy it back into headers. Again a struct copy. */
...
hw_string* create_response_buffer(hw_http_response* response)
{
    ...
    {
        struct http_header header = resp->headers[i];
    ...
    string_from_int(&content_length, resp->body.length, 10);
